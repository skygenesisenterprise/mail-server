...
impl EmailParser {
    ...
    pub fn parse_message(&self, raw_content: &[u8], mailbox_id: Uuid, uid: i32) -> Result<MessageStorage> {
        let message = self.parser.parse(raw_content)
            .ok_or_else(|| crate::error::MailError::InvalidMessage("Failed to parse email message".to_string()))?;
        ...
        let recipients: Vec<String> = message.to()
            .iter()
            .flat_map(|to| to.iter())
            .map(|addr| addr.to_string())
            .collect();
        ...
        let internal_date = message.date()
        ...
        let thread_id = message.header("Message-ID")
    ...
    pub fn extract_attachments(&self, message: &Message) -> Vec<AttachmentInfo> {
        message.attachments()
            .map(|attachment| AttachmentInfo {
                filename: attachment.attachment_name().unwrap_or("unknown").to_string(),
