...
impl PasswordManager {
    ...
    pub fn hash_password(&self, password: &str) -> Result<String> {
        ...
        let password_hash = self.argon2
            ...
            .map_err(|e| MailServerError::Authentication(format!("Failed to hash password: {}", e)))?
            .to_string();
        Ok(password_hash)
    }
    ...
    pub fn verify_password(&self, password: &str, hash: &str) -> Result<bool> {
        let parsed_hash = PasswordHash::new(hash)
            .map_err(|e| MailServerError::Authentication(format!("Invalid password hash: {}", e)))?;
    ...
    pub fn validate_password_strength(&self, password: &str) -> Result<()> {
        if password.len() < 8 {
            return Err(MailServerError::InvalidInput("Password must be at least 8 characters long".to_string()));
        }
    ...
    pub async fn change_password(&self, user_id: Uuid, old_password: &str, new_password: &str) -> Result<()> {
        ...
        if !self.verify_password(old_password, &row.password_hash)? {
            return Err(MailServerError::Authentication("Current password is incorrect".to_string()));
        }
