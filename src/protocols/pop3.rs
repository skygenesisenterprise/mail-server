...
impl ProtocolServer for Pop3Server {
    async fn start(&self) -> Result<()> {
        ...
        info!(
            "Starting POP3 server on {} (plain) and {} (TLS)",
                    ...
                    Ok((stream, addr)) => {
                        ...
                        tokio::spawn(async move {
                            match tls_acceptor.accept(stream).await {
                                Ok(tls_stream) => {
                                    if let Err(e) =
                                        handle_pop3_tls_connection(tls_stream, ctx, cfg).await
                                    {
...
impl Pop3Session {
    ...
    async fn handle_user(&mut self, parts: &[&str]) -> Result<String> {
        if !matches!(self.state, Pop3State::Authorization) {
            return Ok(
                "-ERR Command not permitted when not in AUTHORIZATION state\r\n".to_string(),
            );
        }
    ...
    async fn handle_stat(&self) -> Result<String> {
        match &self.state {
            Pop3State::Transaction(_, messages) => {
                ...
                let total_size: i32 = active_messages.iter().map(|msg| msg.size_bytes).sum();
            ...
            Pop3State::Transaction(_, messages) => {
                if parts.len() > 1 {
                    ...
                    if let Ok(msg_num) = parts[1].parse::<i32>() {
                        if let Some(message) =
                            messages.iter().find(|msg| msg.message_number == msg_num)
                        {
    ...
    async fn handle_top(&self, parts: &[&str]) -> Result<String> {
        match &self.state {
            Pop3State::Transaction(_, messages) => {
                ...
                if let (Ok(msg_num), Ok(line_count)) =
                    (parts[1].parse::<i32>(), parts[2].parse::<i32>())
                {
                    if let Some(message) = messages.iter().find(|msg| msg.message_number == msg_num)
                    {
                        if !self.deleted_messages.contains(&msg_num) {
                            ...
                            let end_line =
                                std::cmp::min(header_end + 1 + line_count as usize, lines.len());
    ...
    async fn handle_quit(&mut self) -> Result<String> {
        match &self.state {
            ...
            Pop3State::Authorization => Ok("+OK POP3 server signing off\r\n".to_string()),
            Pop3State::Update(_, _) => Ok("+OK POP3 server signing off\r\n".to_string()),
        }
    }
